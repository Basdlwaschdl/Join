...
function createTask() {
    if (!button_delay) {
        button_delay = true;
        if (allFilled()) {
            setYouToUserName(task_contacts);
            closeMenu('contacts', 'dropDownContacts')
...
/**
 * change 'You' to the current User Name
 */

function setYouToUserName(array) {
    array.forEach(element => {
        if (element.name == 'You') element.name = user_name;
    });
};
...
function getUserInfo() {
    ...
    if (users[userId]['contacts'] != undefined) delete users[userId]['contacts'];
    contacts.splice(0, 0, users[userId]);
};
